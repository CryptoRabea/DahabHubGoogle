...
import { Event, ServiceProvider, Booking, User, UserRole, BookingStatus, AppSettings, NavItem, HomeSection, Review, Post, Comment } from '../types';
...
const uploadImageToFolder = async (folder: 'Events' | 'Images' | 'Logos' | 'Receipts' | 'Providers', urlOrBase64: string | undefined): Promise<string | undefined> => {
    ...
    try {
        ...
        console.error(`Upload to ${folder} failed:`, e);
        return urlOrBase64;
    }
};
...
class DatabaseService {
  ...
  async socialLogin(providerType: 'google' | 'facebook'): Promise<User> {
    ...
    // Explicitly set Custom Parameters to force selection if browser has multiple accounts
    provider.setCustomParameters({ prompt: 'select_account' });
    
    // Explicit Firebase Popup - User MUST authorize in window
  ...
  async getUser(userId: string): Promise<User | undefined> {
      try { return await this._getUserProfile(userId); } catch { return undefined; }
  }
  ...
  async updateSettings(settings: AppSettings): Promise<void> {
    if (settings.logoUrl && !settings.logoUrl.startsWith('http')) {
        settings.logoUrl = await uploadImageToFolder('Logos', settings.logoUrl) || settings.logoUrl;
    }
  ...
  async updateContentOverride(key: string, value: string): Promise<void> {
      ...
      if (value.startsWith('data:image')) {
          finalValue = await uploadImageToFolder('Images', value) || value;
      }
      const settingsRef = doc(dbFirestore, 'config', 'appSettings');
      await updateDoc(settingsRef, { [`contentOverrides.${key}`]: finalValue });
  }
  ...
  async addEvent(event: Event): Promise<void> {
    const imageUrl = await uploadImageToFolder('Events', event.imageUrl);
  ...
  async updateEvent(updatedEvent: Event): Promise<void> {
    const imageUrl = await uploadImageToFolder('Events', updatedEvent.imageUrl);
  ...
  async deleteEvent(eventId: string): Promise<void> { await deleteDoc(doc(dbFirestore, 'events', eventId)); }
  ...
  async createPost(post: Post): Promise<void> {
    const imageUrl = await uploadImageToFolder('Images', post.imageUrl);
  ...
  async toggleLikePost(postId: string, userId: string): Promise<void> {
    ...
    if (postSnap.exists()) {
        ...
        await updateDoc(postRef, { likes: hasLiked ? arrayRemove(userId) : arrayUnion(userId) });
    }
  }
  ...
  async createBooking(booking: Booking): Promise<void> {
    const receiptUrl = await uploadImageToFolder('Receipts', booking.receiptImage);
  ...
  async toggleSavedEvent(userId: string, eventId: string): Promise<void> {
    ...
    if (userSnap.exists()) {
        ...
        await updateDoc(userRef, { savedEventIds: saved.includes(eventId) ? arrayRemove(eventId) : arrayUnion(eventId) });
    }
  }
  ...
  async getPendingProviders(): Promise<User[]> {
    ...
    const snapshot = await getDocs(q);
    return snapshot.docs.map(d => ({ id: d.id, ...d.data() } as User)).filter(u => u.providerStatus === 'pending' || u.providerStatus === 'payment_review');
  }

  async requestProviderPayment(userId: string): Promise<void> { await updateDoc(doc(dbFirestore, 'users', userId), { providerStatus: 'pending_payment' }); }

  async submitProviderPayment(userId: string, receiptBase64: string): Promise<void> {
      const receiptUrl = await uploadImageToFolder('Receipts', receiptBase64);
      await updateDoc(doc(dbFirestore, 'users', userId), { providerStatus: 'payment_review', subscriptionReceipt: receiptUrl });
  }
  ...
  async approveProvider(userId: string): Promise<User | null> {
    ...
    if (userSnap.exists()) {
        ...
        const newProvider: ServiceProvider = {
            id: userId, name: user.name, serviceType: 'Driver', description: 'Verified Dahab Partner', phone: '', rating: 5.0, imageUrl: 'https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?auto=format&fit=crop&q=80&w=200', isVerified: true
        };
  ...
  async rejectProvider(userId: string): Promise<void> { await updateDoc(doc(dbFirestore, 'users', userId), { role: UserRole.USER, providerStatus: 'rejected' }); }

  async getReviews(itemId: string): Promise<Review[]> {
    const q = query(collection(dbFirestore, 'reviews'), where('itemId', '==', itemId)); 
    const snapshot = await getDocs(q);
    return snapshot.docs.map(d => ({ id: d.id, ...d.data() } as Review)).sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime());
  }

  async addReview(review: Review): Promise<void> { await addDoc(collection(dbFirestore, 'reviews'), review); }
}
