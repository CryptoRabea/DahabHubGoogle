...
import { Calendar, MapPin, Clock, ArrowRight, Sparkles, Car, Briefcase, XCircle, ArrowUp, ArrowDown, Plus, Trash2, EyeOff, Eye, Edit2, X } from 'lucide-react';
...
const CategoryEditModal = ({ category, onSave, onClose, availableRoutes }: { category: any, onSave: (data: any) => void, onClose: () => void, availableRoutes: any[] }) => {
          ...
          <div className="space-y-4">
             ...
             <div>
                ...
                <select 
                  ...
                  className="w-full border border-gray-300 rounded-lg p-2.5 text-gray-900 mb-2 focus:ring-2 focus:ring-dahab-teal/50 outline-none"
                >
                    ...
                    <option value="/more">More & Guide (/more)</option>
                </select>
                <input 
                  value={formData.link} 
                  onChange={e => setFormData({...formData, link: e.target.value})} 
                  ...
                  placeholder="e.g. /events"
                />
                <p className="text-[10px] text-gray-400 mt-1 italic">Note: You can link to any tab path defined in the menu.</p>
             </div>
          </div>
...
const CategoriesSection = ({ id, isActive, onMove, onDelete, onToggle, isEditing, settings, sectionData, onUpdate }: any) => {
    ...
    return (
      <section className={`relative transition-all ${isEditing ? 'border-2 border-blue-400 border-dashed p-4 rounded-xl m-2 bg-blue-50/20' : ''} ${!isActive && isEditing ? 'opacity-40 grayscale' : ''}`}>
        {isEditing && (
            <div className="absolute top-0 right-0 z-50 flex gap-2 -mt-3 mr-4">
                 <div className="bg-blue-600 text-white px-2 py-1 rounded text-xs font-bold uppercase shadow">Categories Block</div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {categories.map((cat: any, idx: number) => (
             <div key={idx} className="relative group">
                <Link to={cat.link} className={`block p-6 rounded-2xl ${cat.color} hover:opacity-90 transition flex flex-col items-center justify-center gap-3 text-center h-40 group-hover:scale-[1.02] transform duration-300 shadow-sm`}>
                ...
                {isEditing && (
                    <div className="absolute top-2 right-2 flex gap-1 z-10 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button 
                            ...
                            className="p-2 bg-white rounded-full shadow-md hover:bg-gray-100 text-gray-700 hover:text-dahab-teal"
                            title="Edit Category"
                        >
                            <Edit2 size={12} />
                        </button>
                        <button 
                            onClick={(e) => { e.preventDefault(); e.stopPropagation(); handleDeleteCategory(idx); }}
                            className="p-2 bg-white rounded-full shadow-md hover:bg-red-50 text-red-400 hover:text-red-500"
          ...
          {isEditing && (
              <button 
                ...
                className="h-40 rounded-2xl border-2 border-dashed border-dahab-teal flex flex-col items-center justify-center gap-2 text-dahab-teal hover:bg-teal-50 transition-colors font-bold group"
              >
                  <div className="p-3 bg-dahab-teal text-white rounded-full shadow-lg group-hover:scale-110 transition-transform">
                    <Plus size={20} />
                  </div>
                  <span>Add Card</span>
              </button>
        ...
        {editingCategoryIndex !== null && (
            <CategoryEditModal 
                ...
                availableRoutes={settings.navigation}
            />
        )}
      </section>
    );
        ...
        <div className="flex justify-between items-end mb-6">
          <div className="flex items-center gap-4">
             ...
             {isEditing && (
               <button 
                onClick={onQuickAdd}
                className="w-10 h-10 bg-dahab-teal text-white rounded-full shadow-lg flex items-center justify-center hover:bg-teal-700 hover:scale-110 transition active:scale-95"
...
const Home: React.FC = () => {
  ...
  const handleEventSubmit = async (eventData: Partial<Event>) => {
    ...
    // Refresh the page data if needed, but db.addEvent updates mock/firebase
    window.location.reload(); // Simple reload to refresh all section data
  };
  ...
  return (
    <div className="space-y-12 pb-20 relative text-gray-900">
