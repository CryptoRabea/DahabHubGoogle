...
const Login: React.FC<LoginProps> = ({ onLogin }) => {
  ...
  // Detect if we are in mock mode (useful for helpful error messaging)
  // We can infer this if db has specific properties or just by catching errors
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Check for common error codes (Firebase or Mock)
      if (code === 'auth/wrong-password' || code === 'auth/invalid-credential') {
        ...
        setError('Application configuration error (Invalid API Key).');
      } else {
    ...
    <div className="flex flex-col items-center justify-center min-h-[80vh] px-4">
      <div className="w-full max-w-md bg-white rounded-3xl shadow-xl overflow-hidden text-gray-900">
        ...
        <div className="bg-gradient-to-r from-dahab-teal to-blue-500 p-8 text-center text-white flex flex-col items-center">
          ...
          {isMockMode && (
            <div className="mt-4 px-3 py-1 bg-white/20 rounded-full text-[10px] font-bold tracking-widest uppercase">
               Offline / Mock Mode
            </div>
          )}
        </div>
        ...
        <div className="p-8">
          ...
          <div className="relative mb-6">
            ...
            <div className="relative flex justify-center">
              <span className="bg-white px-2 text-xs text-gray-400 uppercase">Or use email</span>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {mode === 'signup' && (
              <label className={`flex items-start gap-3 p-4 border rounded-xl cursor-pointer transition animate-fade-in ${isProviderSignup ? 'bg-teal-50 border-dahab-teal' : 'border-gray-200 hover:bg-gray-50'}`}>
                ...
                <div className="flex-1">
                   ...
                   <p className="text-[10px] text-gray-500 mt-1 leading-tight">Create events, offer driving, or list your business. Requires admin approval.</p>
                </div>
              </label>
            )}

            {error && (
              <div className="flex items-start gap-2 p-3 bg-red-50 text-red-600 rounded-xl text-xs animate-fade-in border border-red-100">
            ...
            <button 
              type="submit" 
              disabled={loading}
              className="w-full bg-gray-900 text-white py-4 rounded-xl font-bold hover:bg-gray-800 transition flex justify-center items-center gap-2 shadow-lg"
            >
              {loading ? (
          ...
          {isMockMode && mode === 'login' && (
            <p className="mt-4 text-[10px] text-gray-400 text-center italic">
              Hint: In mock mode, try <b>admin@dahab.com</b> or sign up a new test account.
            </p>
          )}
        </div>
      </div>
